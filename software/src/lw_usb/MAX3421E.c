...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	Status = XSpi_Transfer(&SpiInstance, w_buf, NULL, 2);

	if (Status != XST_SUCCESS) {
		xil_printf("ERR: Function MAXreg_wr failed with status code %d\n", Status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, wr_buf, NULL, nbytes + 1);
	if(Status != XST_SUCCESS) {
		xil_printf("ERR: Function MAXbytes_wr failed with status code %d\n", Status);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int read_val = XSpi_Transfer(&SpiInstance, val, val, 2);
	if (read_val != XST_SUCCESS) {
		xil_printf("ERR: Function MAXreg_rd failed with status code %d\n", Status);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return val[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE rec_buf[nbytes+1];
	rec_buf[0] = reg;

	read_status = XSpi_Transfer(&SpiInstance, rec_buf, rec_buf, nbytes+1);
	if (read_status != XST_SUCCESS){
		xil_printf("ERR: MAXbytes_rd failed with status %d\n", Status);
	}
